...
export class ChatbotAiComponent implements OnInit {
  ...
  async saveConversation(conversation: string) {
    try {
      ...
      const data = JSON.parse(response.body as string); // cast response.body to string
  ...
  async sendVmessage(message: string) {
    const response: HttpResponse<any> = await this.http.post('http://localhost:5000/vmessage', { prompt: message }, { observe: 'response' }).toPromise();
